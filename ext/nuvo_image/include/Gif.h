...
class Gif {
 public:
  ...
  Gif() : frames(new std::vector<GifFrame>()) {}

  const GifFrame& GetFrame(int i) const { return (*frames)[i]; };

  const int GetFrameCount() const { return frames->size(); }

  bool Empty() const { return frames->empty(); }

  void AddFrame(const GifFrame& frame) { frames->push_back(frame); }
