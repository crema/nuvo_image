...
class ImageProcessInput {
 public:
  ImageProcessInput() : type(InvalidInput) {}
  ...
  ImageProcessInput(const cv::Mat& mat) : mat(mat), type(Image) {}

  ImageProcessInput(const Gif& gif) : gif(gif), type(Animation) {}
  ...
  bool Empty() const { return (buffer == nullptr || buffer->empty()) && mat.empty() && gif.Empty(); }
  ...
  const bool IsWritten() const { return written; }
...
class ImageProcess {
 public:
  ImageProcess(const std::shared_ptr<ImageProcessor>& processor) : processor(processor) {}
...
class TwoImageToNullProcess : public ImageProcess {
 public:
  TwoImageToNullProcess(const std::shared_ptr<ImageProcessor>& processor,
                        const std::string& from1,
...
class NullToNullProcess : public ImageProcess {
 public:
  NullToNullProcess(const std::shared_ptr<ImageProcessor>& processor) : ImageProcess(processor) {}
