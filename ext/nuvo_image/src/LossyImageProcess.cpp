...
double LossyImageProcess::GetSsimByQuality(const std::shared_ptr<std::vector<unsigned char>>& buffer,
...
void LossyImageProcess::Encode(const std::shared_ptr<std::vector<unsigned char>>& buffer,
                               const cv::Mat& image,
                               const int quality) {
  buffer->clear();
